...
## 기능 목록
    1. n번째까지의 와인 중 선택할 수 있는 최댓값을 dp로 저장한다.
    2. 2번째까지의 와인은 모두 선택할 수 있다.
    3. N이 3이상인 경우
        현재 와인과 그 전 와인을 연속으로 선택할 경우 -> i, i-1, dp[i-3]
        현재 와인만 선택할 경우 i, dp[i-2]
        현재 와인을 선택하지 않을 경우 dp[i-1]로 세가지의 경우를 구할 수 있다.
        이 중 가장 큰 값을 dp[i]로 저장한다. (dp[0], dp[1], d[2]는 미리 초기화 한다) 
